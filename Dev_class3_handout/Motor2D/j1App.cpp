...
bool j1App::PostUpdate()
{
	...
	for(item = modules.start; item != NULL && ret == true; item = item->next)
	{
		...
		if (canLoad)
		{
			pModule->load(load_node.child(item->data->name.GetString()));
		}
		/*if (canSave)
		{
			pModule->save();
...
{
	savegame.reset();
	pugi::xml_node time_node = savegame.append_child("save");
	bool ret = true;
	p2List_item<j1Module*>* item;
	j1Module* pModule = NULL;
	canSave = true;
	for (item = modules.start; item != NULL && ret == true; item = item->next)
	{
		...
		if (canSave)
		{
			save_node = time_node.append_child(item->data->name.GetString());
			pModule->save();
		}
